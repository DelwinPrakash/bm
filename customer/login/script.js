(function () {
  ...
  function validate() {
    ...
    if (!id) {
      setError(idInput, idError, "Enter your user ID or registered email.");
      ok = false;
      first = first || idInput;
    } else if (id.length < 4) {
      setError(idInput, idError, "That user ID or email looks too short.");
      ok = false;
      first = first || idInput;
    } else {
  ...
  idInput.addEventListener("input", function () {
    if (idInput.getAttribute("aria-invalid")) setError(idInput, idError, "");
  });
  pwInput.addEventListener("input", function () {
    if (pwInput.getAttribute("aria-invalid")) setError(pwInput, pwError, "");
  });
